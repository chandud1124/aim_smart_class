...
void loop()
{
  ...
  else
  {
    if (!ws.isConnected())
    {
      ...
      // Debug: If not identified for >30s, print log and retry identify
      if (!identified && now - lastIdentifyAttempt > 30000) {
        Serial.println("[DEBUG] Still not identified after 30s, retrying...");
        identify();
      }
    }
    else
