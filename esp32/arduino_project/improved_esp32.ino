...
struct SwitchConfig {
    ...
    int manualGpio;        // Manual switch GPIO pin
...
void setupRelays() {
    LOGI("Setting up relays and manual switches");

    // Setup GPIO pins for each switch
    for (auto& sw : switchesLocal) {
        // Setup relay pin
        pinMode(sw.gpio, OUTPUT);
        digitalWrite(sw.gpio, sw.state ? RELAY_ON_LEVEL : RELAY_OFF_LEVEL);

        // Setup manual switch pin (if configured)
        if (sw.manualGpio != -1) {
            ...
            LOGI("Setup relay GPIO %d (manual pin %d, active %s) - %s",
                 sw.gpio, sw.manualGpio,
                 sw.manualActiveLow ? "LOW" : "HIGH",
                 sw.name.c_str());
        } else {
            LOGI("Setup relay GPIO %d (no manual switch) - %s",
                 sw.gpio, sw.name.c_str());
        }
    }
...
void handleWebSocketMessage(uint8_t* payload, size_t length) {
    ...
    if (strcmp(type, "identified") == 0) {
        ...
        // Check if backend provides switch configuration
        JsonArray switches = doc["switches"];
        if (switches.size() > 0) {
            LOGI("Received switch configuration from backend (%d switches)", switches.size());

            // Clear default configuration and use backend config
            switchesLocal.clear();

            // Load switch configuration from backend
            ...
            // Re-setup relays with new configuration
            setupRelays();
            LOGI("Switched to backend configuration");
        } else {
            LOGI("No switch configuration from backend, using defaults");
        }
