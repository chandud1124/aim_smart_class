...
export const noticesAPI = {
  ...
  submitNotice: (noticeData: FormData) =>
    api.post('/notices/submit', noticeData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    }),

  // Get user's submitted notices
  getMyNotices: () => api.get('/notices/my-notices'),

  // Admin: Get pending notices
  getPendingNotices: () => api.get('/notices/pending'),

  // Admin: Approve notice
  approveNotice: (noticeId: string) =>
    api.put(`/notices/${noticeId}/approve`),

  // Admin: Reject notice
  rejectNotice: (noticeId: string, data: { reason: string }) =>
    api.put(`/notices/${noticeId}/reject`, data),
  ...
  getAllNotices: (params?: {
    ...
    page?: number;
    limit?: number;
  }) => api.get('/notices/all', { params }),

  // Create schedule for approved notice
  scheduleNotice: (noticeId: string, scheduleData: {
  ...
  }) => api.post(`/notices/${noticeId}/schedule`, scheduleData),

  // Get schedules for device
  getDeviceSchedules: (deviceId: string) =>
    api.get(`/notices/device/${deviceId}/schedules`),

  // Register display device
  registerDisplayDevice: (deviceData: {
    deviceId: string;
    name: string;
    location?: string;
    capabilities: {
  ...
  }) => api.post('/notices/devices/register', deviceData),

  // Get all display devices
  getDisplayDevices: () => api.get('/notices/devices'),

  // Set default notice for device
  setDeviceDefaultNotice: (deviceId: string, noticeId: string) =>
    api.put(`/notices/devices/${deviceId}/default`, { noticeId }),
};
